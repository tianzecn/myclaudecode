...
IDE开发环境快速安装命令，支持一键安装或更新现有环境。

## 当调用此命令时：

1. 检查是否已经安装过BMad-Method
...
if [ -f ".bmad-core/install-manifest.yaml" ] || [ -d ".claude/commands/BMad" ]; then
    ...
    if [ -d ".git" ]; then
        echo "🔄 正在拉取最新代码..."
        git pull || echo "⚠️  Git拉取失败，继续安装流程"
    fi
