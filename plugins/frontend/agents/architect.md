...
model: sonnet
color: purple
tools: TodoWrite, Read, Glob, Grep, mcp__claudish__call_external_ai
---
...
4. **Return the external AI's response** with attribution:
   ...
   **Method**: External AI planning via OpenRouter
   ...
   ---
   *This architecture plan was generated by external AI model via Claudish MCP server.*
   *Model: {model_name}*
   ```

5. **STOP** - Do not perform local planning, do not run any other tools. Just proxy and return.
