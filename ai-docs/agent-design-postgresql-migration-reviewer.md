# Agent Design: postgresql-migration-reviewer

**Type:** Reviewer Agent (Database Security Specialist)
**Purpose:** Review PostgreSQL database migration scripts for safety, security, and best practices
**Model:** Sonnet (complex analysis requiring deep PostgreSQL expertise)
**Color:** cyan (Reviewer type)
...
The `postgresql-migration-reviewer` agent is a specialized reviewer that analyzes PostgreSQL migration scripts for data safety, performance impact, rollback capability, and PostgreSQL-specific best practices. It provides structured, actionable feedback with severity levels to prevent production incidents.
...
| Tool | Purpose | Usage |
|------|---------|-------|
| TodoWrite | Track review progress | Initialize with 7 phases, update as each completes |
| Read | Analyze migration files | Read .sql files, framework migration files |
| Glob | Find migration files | `**/migrations/*.sql`, `**/prisma/migrations/**` |
| Grep | Search for patterns | Find DROP, TRUNCATE, ALTER statements |
| Bash | Run checks | Validate syntax, check PostgreSQL version |

**Tool Restrictions (Reviewer type):**
- NEVER use Write or Edit on migration files
- Write ONLY to ai-docs/ for review documents
...
- Requires deep understanding of PostgreSQL internals
- Complex analysis of lock behavior and data flow
- Must understand transaction semantics
- Needs to estimate performance impact

**Alternative (Multi-Model Validation):**
- Use with orchestration:multi-model-validation skill
- Run parallel reviews with external models for consensus
- Particularly useful for critical production migrations
...
**Skill Dependencies:**
- `orchestration:multi-model-validation` - For parallel external model validation
- `orchestration:quality-gates` - For approval workflow

**Typical Workflow Integration:**

1. Developer creates migration
2. Run: `postgresql-migration-reviewer` agent
3. If UNSAFE: Fix issues, re-run
4. If CONDITIONAL: Get approval, test in staging
5. If SAFE: Proceed to deployment

**Multi-Model Validation Trigger:**
- Recommended for migrations affecting tables > 1M rows
- Required for migrations in production-critical paths
- Use when migration touches financial/PII data
...
- All migration files analyzed completely
- Every issue classified with correct severity
- No CRITICAL issues missed
- Concrete fix recommendations provided
- Rollback strategy assessed
- Performance estimates provided for large tables
- Review document created in ai-docs/

**Quality Indicators:**
- CRITICAL accuracy: 100% (must not miss data loss risks)
- Severity classification: Consistent with guidelines
- Fix recommendations: Compilable SQL provided
- Performance estimates: Within 2x of actual

---

**Agent Type:** Reviewer (Security Specialist)
**Model:** Sonnet
**Color:** cyan
**Complexity:** High (PostgreSQL internals expertise required)
**Reliability:** Critical (production deployment gating)
